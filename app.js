...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (!localStorage.getItem('_defaultsSet')) {
    ...
    localStorage.setItem('themeColor', '#475569');
...
function applyIconStyle() {
  ...
  var themeColor = localStorage.getItem('themeColor') || '#475569';
  var isGray = themeColor === '#475569';
...
var TITLE_COLORS = [
  ...
  '#f1f5f9','#cbd5e1','#94a3b8','#475569','#334155',
];
...
// 6가지 고정 테마 색상 (그레이 1 + 채도 낮은 5, 더 어두운 톤)
var THEME_COLORS = [
  '#475569', // gray (slate-600)
  '#b91c1c', // deep rose (red-700)
  '#c2410c', // burnt orange (orange-700)
  '#15803d', // forest green (green-700)
  '#1d4ed8', // royal blue (blue-700)
  '#6d28d9'  // deep violet (violet-700)
];
...
function startQrScanner() {
  ...
  qrScanner.start({ facingMode: 'environment' }, { fps: 10, qrbox: 200 }, decoded => {
    ...
    else if (_cl === 'zh') _cm = '<p style="color:#475569;font-size:13px;text-align:center;">需要相机权限</p>';
    else if (_cl === 'ja') _cm = '<p style="color:#475569;font-size:13px;text-align:center;">カメラの権限が必要です</p>';
    else _cm = '<p style="color:#475569;font-size:13px;text-align:center;">카메라 권한이 필요합니다</p>';
    wrap.innerHTML = _cm;
  });
}
...
function _resetTimerModal() {
  ...
  if (desc) {
    ...
    desc.style.color = '#475569'; desc.style.fontSize = '12px';
  }
...
function renderStatsUI() {
  ...
  var dateCl  = isDark ? '#475569' : '#475569';
  ...
  Object.keys(STAT_CATS).forEach(function(k) {
    ...
    var col = active ? "#fff" : "var(--text,#475569)";
    var btn = document.createElement("button");
  ...
  var chartHtml = '<div style="background:' + boxBg + ';border:1.5px solid ' + boxBd + ';border-radius:16px;padding:16px;margin-bottom:16px;"><div style="font-size:14px;font-weight:700;color:' + titleCl + ';">' + cat.emoji + ' ' + statLabel(curSC) + '</div><div style="font-size:11px;color:#475569;margin-bottom:12px;">' + (__T('Unit: ','단위: ','单位: ','単位: ')) + statUnit(curSC) + '</div>';
  if (entries.length === 0) {
    chartHtml += '<div style="text-align:center;color:#475569;font-size:13px;padding:30px 0;">데이터가 없어요.<br>+ 입력으로 추가해보세요!</div>';
  } else {
    chartHtml += '<canvas id="sCanvas" style="width:100%;"></canvas>';
  }
  chartHtml += "</div>";

  var listHtml = '<div style="font-size:13px;font-weight:700;color:#475569;margin-bottom:8px;">' + (__T('Recent Records','최근 기록','最近记录','最近の記録')) + '</div>';
  entries.slice().reverse().slice(0,10).forEach(function(e, i) {
    var origIdx = entries.length - 1 - i;
    var valDisplay = (curSC === 'bp' && e.dia != null)
      ? '<span style="font-size:15px;font-weight:700;color:' + cat.color + ';">' + e.value + '/' + e.dia + ' <small style="font-size:11px;color:#475569;">mmHg</small></span>'
      : '<span style="font-size:15px;font-weight:700;color:' + cat.color + ';">' + e.value + ' <small style="font-size:11px;color:#475569;">' + cat.unit + '</small></span>';
...
function openSM() {
  ...
  var subCl  = isDark ? '#475569' : '#475569';
  var cancelBg = isDark ? '#2A2A2A' : '#f1f5f9';
  var cancelCl = isDark ? '#475569' : '#475569';
